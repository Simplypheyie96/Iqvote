 ...
 *
 * The list is `w-max min-w-full` on purpose: seven tabs never fit a phone, so
 * the row scrolls inside an edge-bleeding wrapper and a half-visible tab
 * becomes the honest signal that there is more this way.
 */
export const TAB_LIST_CLASS =
  'h-11 w-max min-w-full gap-1 rounded-full bg-muted/70 p-1 inset-ring-1 inset-ring-border/60';
...
export const SUBTAB_LIST_CLASS =
  'h-10 w-max gap-1 rounded-full bg-muted/70 p-1 inset-ring-1 inset-ring-border/60';
